...
const InitiativeShow = () => {
  ...
  const [held, setHeld] = useState(initiative?.heldAmount ?? 0);
  const [showConfetti, setShowConfetti] = useState(false);
  const [currentStatus, setCurrentStatus] = useState(initiative?.status ?? 'open');
                  ...
                  {initiative.updates.map((update, i) => (
                    <div key={i} className="flex gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">{update.date.toLocaleDateString()}</p>
                      </div>
                    </div>
                  ))}
                </div>
